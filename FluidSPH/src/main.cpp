...
private:
    ...
    {
        // TODO:
        // Pick the position of our particle to find its neighbors
            ...
            for (auto point : neighbors) {
                for (auto particle : _pidxInGrid[idx1d(std::get<0>(point), std::get<1>(point))]) {
                    if (particle != i) {
                        ...
                        _acc[i] -= _m0 * _kernel.grad_w(_pos[i] - _pos[particle]) * (_p[i] / _d[i] / _d[i] + _p[particle] / _d[particle] / _d[particle]);
                    }
    ...
    void applyViscousForce()
    {
        ...
        for (tIndex i = 0; i < _pos.size(); i++) {
            ...
            for (auto point : neighbors) {
                for (auto particle : _pidxInGrid[idx1d(std::get<0>(point), std::get<1>(point))]) {
                    if (particle != i) {
                        _acc[i] += 2 * _nu * _m0 / _d[particle] * (_vel[particle] - _vel[i]) * _kernel.grad_w(_pos[i] - _pos[particle]) * (_pos[i] - _pos[particle]) / ((_pos[i] - _pos[particle]) * (_pos[i] - _pos[particle]) + 0.01 * _h * _h);
                        //aux = (_m0/_d[particle]) * ((_p[i] / (_d[i] * _d[i])) + (_p[particle] / (_d[particle] * _d[particle]))) * _kernel.grad_w(_pos[i] - _pos[particle]);
                    }

                }
            }
            //_acc[i] += (2 * _m0 * _nu)*aux;
